...
const register1 = function (plugin, options, next) {
...
const register = function (server, options) {
    ...
    server.route([{
        ...
        config: {
            ...
            handler: (request, reply) => {
                ...
                if (settings.cache) {
                    /*eslint no-unused-vars:0 */
                    server.methods.getSwaggerJSON(settings, request, (err, json, cached, report) => {
                        /* $lab:coverage:off$ */
                        if (err) {
                            reply(err);
                            /* $lab:coverage:on$ */
    ...
    if (settings.documentationPage === true || settings.swaggerUI === true) {
        ...
        // make sure we have other plug-in dependencies
        server.dependency(['inert', 'vision'], (pluginWithDependencies, nextWithDependencies) => {
...
const dependencies = ['inert', 'vision'];
const once = true;
